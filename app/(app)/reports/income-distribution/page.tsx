...
type IncomeDistributionPageProps = {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
};

type IncomeDistributionRow = {
  accountKey: string;
  accountDescription: string;
  amount: number;
  percentage: number;
  movements: IncomeDistributionMovement[];
};

type IncomeDistributionMovement = {
  id: string;
  description: string;
  amount: number;
};
...
function isIncomeMovement(movement: TreasuryBalanceMovement) {
...
function buildIncomeMovementsByAccount(
  movements: TreasuryBalanceMovement[]
) {
  const movementsByAccount = new Map<string, IncomeDistributionMovement[]>();

  movements.filter(isIncomeMovement).forEach((movement, index) => {
...
function buildIncomeDistributionRows(
  movements: TreasuryBalanceMovement[]
): IncomeDistributionRow[] {
  const balanceRows = buildTreasuryBalanceRows(movements);
  const movementsByAccount = buildIncomeMovementsByAccount(movements);
  const incomeRows = balanceRows
    .map((row) => ({
      accountKey: row.accountId || row.accountNo || row.accountDescription,
      accountDescription: row.accountDescription || "-",
      amount: row.realIncome + row.expectedIncome,
      percentage: 0,
      movements:
  ...
  const totalAmount = incomeRows.reduce(
...
function getModeHref({
  ...
  return Object.keys(query).length > 0
    ? {
        pathname: "/reports/income-distribution",
        query,
      }
...
export default async function IncomeDistributionPage({
  ...
  if (activeCompany) {
    try {
      ...
      throw new Error(`${dict.reports.incomeDistributionReadError}: ${message}`);
    }
  }

  const rows = buildIncomeDistributionRows(movements);
  ...
  return (
    <section className="max-w-4xl space-y-4">
      ...
      {!activeCompany ? (
        ...
        <div className="rounded-xl border border-app-border bg-app-soft p-4 text-sm text-app-muted">
          {dict.reports.emptyIncomeDistribution}
        </div>
      ) : (
        <div className="space-y-3">
          <div className="flex max-w-[560px] items-center justify-between gap-3 border-b border-app-border pb-1.5 text-xs font-semibold text-primary-app sm:text-sm">
            <span>{dict.reports.totalIncome}</span>
